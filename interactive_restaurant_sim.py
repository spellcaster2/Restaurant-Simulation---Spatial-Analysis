...
# --- Soft reset helpers --
# =========================
def new_log_filenames():
    """Create fresh log filenames with a timestamp suffix."""
    ts = datetime.now().strftime("%Y%m%d_%H%M%S")
    return f"sim_log_{ts}.csv", f"kpi_log_{ts}.csv"

def soft_reset_runtime_state_preserve_layout():
    """
    Clear runtime state (agents, counters, queues) but KEEP current LAYOUT and dimensions.
    Rebuild obstacle map from the existing layout. Prepare new empty logs (new files).
    """
    global all_customers, all_waiters, customers_served_count, seated_customer_wait_times
    global EVENT_LOG, OBSTACLE_MAP, LOG_FILE, KPI_LOG_FILE

    # Clear live agents and metrics
    all_customers[:] = []
    all_waiters[:] = []
    customers_served_count = 0
    seated_customer_wait_times[:] = []
    ...
    # Rebuild obstacle map using the same layout
    OBSTACLE_MAP = create_obstacle_map(resolution=OBSTACLE_RES)

    # Fresh log files
    LOG_FILE, KPI_LOG_FILE = new_log_filenames()
    init_logger()  # write headers to new files

def make_param_input_boxes():
    """Recreate parameter input boxes, prefilled with current values."""
    return {
        ...
        'window_rate':  InputBox(CANVAS_WIDTH + 25, 610, 200, 32,
                                 str(LAYOUT.get('arrival_rates', {}).get('window', 0))),
        'aisle_rate':   InputBox(CANVAS_WIDTH + 25, 680, 200, 32,
                                 str(LAYOUT.get('arrival_rates', {}).get('aisle', 0))),
    }
...
def main():
    ...
    global LOG_FILE, KPI_LOG_FILE  # we'll update these on Z
    ...
    input_boxes = make_param_input_boxes()
    ...
    help_lines = [
        ...
        "RUNNING mode: BACKSPACE to reset layout | Z to stop & return to Parameters (keep layout, new logs)"
    ]
    ...
    while running:
        for event in pygame.event.get():
            ...
            if app_state == 'SETUP_LAYOUT':
                ...
                # NEW: Z hotkey -> stop sim, keep layout, go to parameters, new logs
                if event.type == pygame.KEYDOWN and event.key == pygame.K_z:
                    # discard env so all processes stop immediately
                    env = None
                    # clear runtime state, keep layout, create new log files
                    soft_reset_runtime_state_preserve_layout()
                    # rebuild parameter inputs with current values
                    input_boxes = make_param_input_boxes()
                    # switch UI state & caption
                    app_state = 'SETUP_PARAMETERS'
                    pygame.display.set_caption("Restaurant Simulation — Adjust Parameters (Layout Preserved)")
                    logging.info("=== Z pressed: stopped simulation, returned to parameters with same layout. New log files created. ===")
                ...
                if event.type == pygame.KEYDOWN and event.key == pygame.K_BACKSPACE:
                    ...
                    # Also reset input boxes to defaults
                    input_boxes = make_param_input_boxes()
        ...
        if app_state == 'SETUP_LAYOUT':
            ...
            if SHOW_HELP:
                help_lines = [
                    ...
                    "Tip: Press Z while running to come back here with layout preserved and new logs."
                ]
                for i, line in enumerate(help_lines):
        ...
        if app_state == 'RUNNING':
            ...
            screen.blit(font_bold.render("Press [Z] Stop to Params (keep layout, new logs) | [BACKSPACE] Restart layout", True, RED), (CANVAS_WIDTH + 5, SCREEN_HEIGHT - 50))
